    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-4">
            <div class="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 bg-gradient-to-br from-green-400 to-green-700 shadow-lg shadow-green-600/30 animate-pop">
                <i data-lucide="sprout" class="w-5 h-5 text-white" aria-hidden="true"></i>
            </div>
            <div>
                <h2 class="text-lg font-extrabold text-gray-900">Daftar Petani</h2>
                <p class="text-xs text-gray-400">Buat akun petani hidroponik</p>
            </div>
        </div>
        ...
        <div class="bg-green-50 border border-green-200 rounded-2xl px-4 py-3 text-xs text-green-700 leading-relaxed flex items-start gap-2">
            <i data-lucide="info" class="w-4 h-4 shrink-0 mt-0.5" aria-hidden="true"></i>
            <span>{{ __('Akun Anda akan diverifikasi oleh Admin sebelum dapat berjualan.') }}</span>
        </div>
    </div>
        ...
        <div>
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <x-text-input id="nama_kebun" type="text" name="nama_kebun" :value="old('nama_kebun')" placeholder="Contoh: Hidroponik Sejahtera" />
                    <x-input-error :messages="$errors->get('nama_kebun')" class="mt-1" />
                </div>

                <div>
                    <x-input-label for="lokasi_kebun" :value="__('Lokasi Kebun')" />
                    <x-text-input id="lokasi_kebun" type="text" name="lokasi_kebun" :value="old('lokasi_kebun')" placeholder="Kota / Kecamatan / Alamat kebun" />
                    <x-input-error :messages="$errors->get('lokasi_kebun')" class="mt-1" />
                </div>

                <div>
                    <x-input-label for="deskripsi_kebun" :value="__('Deskripsi Singkat')" />
                    <textarea id="deskripsi_kebun" name="deskripsi_kebun" rows="3"
                        class="form-input resize-none"
                        placeholder="Ceritakan sedikit tentang kebun hidroponik Anda...">{{ old('deskripsi_kebun') }}</textarea>
                    <x-input-error :messages="$errors->get('deskripsi_kebun')" class="mt-1" />
                </div>
            </div>
        </div>
