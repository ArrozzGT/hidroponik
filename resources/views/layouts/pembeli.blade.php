...
<div x-data="pembeliLayout()" x-init="init()" class="min-h-screen flex">
    ...
    <aside id="sidebar"
        ...
        <div class="flex items-center gap-2.5 px-4 h-14 shrink-0 border-b border-gray-100">
            ...
            <span class="ml-auto text-[10px] font-medium text-teal-600 bg-teal-50 px-2 py-0.5 rounded-md">Pembeli</span>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
            ...
            @foreach($pembeliNav as $item)
                <a href="{{ route($item['route']) }}"
                   class="flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg transition-colors
                          {{ request()->routeIs($item['route']) ? 'bg-teal-50 text-teal-700 font-medium border-l-2 border-teal-500' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50' }}">
            ...
            <div class="text-[10px] font-medium text-gray-400 uppercase tracking-wider px-3 pt-5 pb-1.5">Akun</div>

            <a href="{{ route('profile.edit') }}"
               class="flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg transition-colors
                      {{ request()->routeIs('profile.edit') ? 'bg-teal-50 text-teal-700 font-medium border-l-2 border-teal-500' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50' }}">
            ...
            <div class="px-3 pt-4 flex items-center gap-2 text-xs text-gray-400">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('cart.index') }}"
                       ...
                       title="Keranjang">
                        ...
                        @if(($cartCount ?? 0) > 0)
                            <span class="absolute top-1 right-1 w-3.5 h-3.5 bg-teal-500 text-white text-[8px] font-bold rounded-full flex items-center justify-center">{{ min($cartCount ?? 0, 9) }}</span>
                        @endif
                    </a>
                    ...
                    <x-dropdown align="right" width="52">
                        ...
                        <x-slot name="content">
                            <div class="px-4 py-3 border-b border-gray-100">
                                ...
                                <span class="inline-flex mt-1.5 px-2 py-0.5 text-[10px] font-medium bg-teal-50 text-teal-700 rounded-md">Pembeli</span>
                            </div>
