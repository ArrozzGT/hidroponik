...
<div x-data="pembeliLayout()" x-init="init()" class="min-h-screen flex">
    ...
    <aside id="sidebar"
        ...
        <div class="sidebar-header flex items-center gap-2.5 px-4 h-14 shrink-0">
            <div class="w-7 h-7 rounded-lg" style="background:linear-gradient(135deg,#14b8a6,#0d9488);">
            ...
            <span class="ml-auto text-[10px] font-medium text-harvest-sky" style="background:rgba(14,165,233,0.15);padding:2px 8px;border-radius:6px;">Pembeli</span>
        </div>
        ...
        <nav @click="sidebarOpen = false" class="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
                ...
                <a href="{{ route($item['route']) }}"
                   class="nav-item flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg
                    ...
                    @if(!empty($item['badge']) && $item['badge'] > 0)
                        <span class="ml-auto min-w-[18px] h-[18px] px-1 rounded-md text-[10px] font-bold" style="background:rgba(14,165,233,0.2);color:#38bdf8;display:flex;align-items:center;justify-content:center;">{{ $item['badge'] > 9 ? '9+' : $item['badge'] }}</span>
                    @endif
                </a>
            @endforeach
            ...
            <div class="px-3 pt-4 flex items-center gap-2 text-xs text-white/40">
                <span class="w-2 h-2 rounded-full bg-harvest-leaf"></span>
        ...
        <div class="sidebar-header px-4 py-3 shrink-0">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full" style="background:linear-gradient(135deg,#14b8a6,#0d9488);display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:600;">
                ...
                <span class="px-1.5 py-0.5 text-[10px] font-medium text-harvest-sky" style="background:rgba(14,165,233,0.15);border-radius:6px;">Pembeli</span>
            </div>
        </div>
    </aside>

    {{-- Main Content --}}
    <div class="flex-1 flex flex-col min-w-0 min-h-screen content-area" style="background-image:radial-gradient(at 20% 30%, rgba(20,184,166,0.06) 0px, transparent 50%),radial-gradient(at 80% 20%, rgba(236,72,153,0.03) 0px, transparent 50%);" :class="sidebarOpen ? 'lg:ml-64 xl:ml-72' : 'lg:ml-0'">
        {{-- Topbar --}}
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('cart.index') }}"
                       ...
                       title="Keranjang">
                        ...
                        @if(($cartCount ?? 0) > 0)
                            <span class="cart-badge-pulse absolute top-1 right-1 w-3.5 h-3.5" style="background:rgba(20,184,166,0.2);color:#2dd4bf;text-shadow:0 0 8px rgba(20,184,166,0.5);font-size:8px;font-weight:700;border-radius:999px;display:flex;align-items:center;justify-content:center;">{{ min($cartCount ?? 0, 9) }}</span>
                        @endif
                    </a>
                    ...
                    <x-dropdown align="right" width="52">
                        <x-slot name="trigger">
                            <button class="flex items-center gap-2 px-3 py-1.5 text-sm text-white/60 hover:text-white transition-colors">
                                <div class="w-7 h-7 rounded-full" style="background:linear-gradient(135deg,#14b8a6,#0d9488);display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:600;">
                                    {{ strtoupper(substr(Auth::user()->name ?? 'B', 0, 1)) }}
                                </div>
                        ...
                        <x-slot name="content">
                            <div class="px-4 py-3 border-b border-white/10" style="background:rgba(11,23,16,0.95);">
                                ...
                                <span class="inline-flex mt-1.5 px-2 py-0.5 text-[10px] font-medium text-harvest-sky" style="background:rgba(14,165,233,0.15);border-radius:6px;">Pembeli</span>
                            </div>
...
<script>
    let _pembeliResizeHandler = null;
    function pembeliLayout() {
        return {
            sidebarOpen: window.innerWidth >= 1024,
            init() {
                if (_pembeliResizeHandler) window.removeEventListener('resize', _pembeliResizeHandler);
                _pembeliResizeHandler = () => {
                    if (window.innerWidth >= 1024) this.sidebarOpen = true;
                    else this.sidebarOpen = false;
                };
                window.addEventListener('resize', _pembeliResizeHandler);
            }
        };
    }
</script>
...
<button class="back-to-top fixed bottom-6 right-6 z-50 w-10 h-10 rounded-full" style="background:linear-gradient(135deg,#14b8a6,#0d9488);color:#fff;box-shadow:0 4px 20px rgba(20,184,166,0.3);display:flex;align-items:center;justify-content:center;transition:all 0.3s;opacity:0;visibility:hidden;" aria-label="Kembali ke atas">
