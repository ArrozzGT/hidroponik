...
@if ($total > 1)
    <nav {{ $attributes->merge(['class' => 'flex items-center gap-1']) }}>
        <a href="#"
            data-page="{{ $current - 1 }}"
            class="inline-flex items-center justify-center h-9 w-9 rounded-md text-sm transition-colors {{ $current <= 1 ? 'opacity-50 pointer-events-none text-gray-300' : 'text-gray-600 hover:bg-gray-50' }}"
        ...
        <a href="#"
            data-page="{{ $current + 1 }}"
            class="inline-flex items-center justify-center h-9 w-9 rounded-md text-sm transition-colors {{ $current >= $total ? 'opacity-50 pointer-events-none text-gray-300' : 'text-gray-600 hover:bg-gray-50' }}"
            onclick="event.preventDefault(); window.dispatchEvent(new CustomEvent('page-change', {detail: {page: {{ $current + 1 }}}}))"
        >
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
