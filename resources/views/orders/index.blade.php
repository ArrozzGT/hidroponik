    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-600 mt-0.5">Semua pesanan yang pernah Anda buat</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white border border-gray-100 rounded-xl overflow-hidden">
                <div class="p-6">
                        ...
                        @foreach($statuses as $key => $label)
                            <a href="{{ route('orders.index', ['status' => $key === 'all' ? null : $key]) }}"
                               class="whitespace-nowrap px-4 py-2 text-sm font-medium rounded-lg transition-colors
                                      {{ $currentStatus === $key ? 'bg-emerald-100 text-emerald-700' : 'text-gray-600 hover:text-gray-700 hover:bg-gray-50' }}">
                                {{ $label }}
                            </a>
                        @endforeach
                    </div>
                    ...
                    <form method="GET" action="{{ route('orders.index') }}" class="mb-5">
                        <div class="relative">
                            <i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-600" aria-hidden="true"></i>
                    ...
                    @else
                        <div class="hidden md:block overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-100">
                                <thead>
                                    <tr>
                                        <th class="px-5 py-3.5 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Order #</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Tanggal</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Total</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Status Pesanan</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Pembayaran</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-50">
                                    @foreach($orders as $order)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-5 py-4 text-sm text-gray-600">{{ $order->created_at->format('d M Y') }}</td>
                                ...
                                <div class="border border-gray-100 rounded-xl p-4">
                                    <div class="flex items-start justify-between mb-3">
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-600 mt-0.5">{{ $order->created_at->format('d M Y') }}</p>
                                        </div>
