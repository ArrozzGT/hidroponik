<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-5" enctype="multipart/form-data">
        ...
        <div>
            ...
            @if($user->foto)
                <div class="mt-2 mb-3">
                    <img src="{{ asset('storage/' . $user->foto) }}" alt="Avatar" class="w-20 h-20 rounded-2xl object-cover shadow-soft">
                </div>
            @endif
            <div class="mt-1 flex items-center justify-center px-4 py-5 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50 hover:bg-gray-100/50 transition-colors cursor-pointer">
